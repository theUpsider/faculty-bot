...
#[poise::command(
    ...
    description_localized("de", "Zeige die E-Mail Adresse, mit der ein Nutzer sich verifiziert hat"),
...
pub async fn getmail(
    ...
    if let Some(db_usr) = db_user {
        ctx.say(&format!("{} is verified with {}", user.tag(), db_usr.user_email)).await.map_err(Error::Serenity)?;
    } else {
        ctx.say(&format!("{} is not verified", user.tag())).await.map_err(Error::Serenity)?;
    }
